export default {
  id: 'dwi-satya',
  title: 'Dwi Satya',
  preface:
    'Pengertian dari Dwi Satya adalah Dwi: dua, Satya: Kesetiaan. Artinya adalah dua kesetiaan yang harus di penuhi oleh atau dipatuhi oleh setia para anggota Pramuka Siaga.',
  data: {
    origin: {
      preface: 'Demi kehormatanku aku berjanji akan bersungguh-sungguh :',
      data: [
        'Menjalankan kewajibanku terhadap Tuhan, Negara Kesatuan Republik Indonesia, dan mengikuti tata krama keluarga.',
        'Setiap hari berbuat kebaikan.',
      ],
    },
  },
};
